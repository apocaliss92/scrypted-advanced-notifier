...
import {
  AlarmEvent,
  getAlarmModeSequences,
  getAlarmSequenceNames,
  getAlarmSettings,
  getAlarmWebhookUrls,
  getModeEntity,
} from "./alarmUtils";
...
export class AdvancedNotifierAlarmSystem
  extends ScryptedDeviceBase
  implements SecuritySystem, Settings
{
  initStorage: StorageSettingsDict<StorageKeys> = {
    ...
    useRuleNotifiers: {
      ...
      description:
        "If checked, the notifiers will be automatically picked from the active rules, with same settings",
      type: "boolean",
      defaultValue: false,
      immediate: true,
    },
    ...
    armingMessage: {
      ...
      description:
        "Message sent when the alarm is armed. Available placeholders are ${mode}, ${bypassedDevices}, ${activeDevicesAmount}",
      type: "textarea",
      group: "Texts",
      defaultValue:
        'Alarm armed in "${mode}" mode. ${activeDevicesAmount} active devices. Bypassed devices: ${bypassedDevices}',
    },
    preActivationStartMessage: {
      title: "Preactivation message",
      description:
        "Message sent when the alarm starts the preactivation. Available placeholders are ${mode}, ${bypassedDevices}, ${activeDevicesAmount} and ${seconds}",
      type: "textarea",
      group: "Texts",
      defaultValue:
        'Alarm will be armed in "${mode}" mode in ${seconds} seconds. ${activeDevicesAmount} active devices. Bypassed devices: ${bypassedDevices}',
    },
    armingErrorMessage: {
      title: "Arming message",
      description:
        "Message sent when the alarm cannot be armed. Available placeholders are ${mode} and ${blockingDevices}",
      type: "textarea",
      group: "Texts",
      defaultValue:
        'Alarm cannot be armed in "${mode}" mode. Blocking devices: ${blockingDevices}',
    },
    triggerMessage: {
      title: "Trigger message",
      description:
        "Message sent when the alarm is triggered. Available placeholders are ${triggerDevices}",
      type: "textarea",
      group: "Texts",
    ...
    defuseMessageAutomatic: {
      ...
      description:
        "Message sent when the alarm is automatically disarmed while triggered",
      type: "string",
      group: "Texts",
  ...
  constructor(
    nativeId: string,
    private plugin: AdvancedNotifierPlugin,
  ) {
    super(nativeId);
    const logger = this.getLogger();
    ...
    this.startStop(this.plugin.storageSettings.values.pluginEnabled)
      .then()
      .catch(logger.log);
  }
            ...
            if (
              ...
              await subscribeToAlarmSystemMqttTopics({
                ...
                modeSwitchCb: async (mode) => {
                  const now = Date.now();
                  if (
                    !this.lastMqttCommandReceived ||
                    now - this.lastMqttCommandReceived > 1000 * 2
                  ) {
                    logger.log(
                      `Setting mode to ${mode} (currently ${this.securitySystemState.mode})`,
                    );
  ...
  async refreshSettings() {
    const sequenceNames = getAlarmSequenceNames({
      pluginStorage: this.plugin.storageSettings,
    });
    ...
    dynamicSettings.push(
      ...getAlarmSettings({ mode: SecuritySystemMode.Disarmed, sequenceNames }),
    );
  ...
  async updateMqtt(props: {
    mode: string;
    info?: { activeDevices: string[]; bypassedDevices: string[] };
  }) {
  ...
  async sendNotification(props: {
    ...
    try {
      ...
      const modeText =
        mode === SecuritySystemMode.AwayArmed
          ...
          : mode === SecuritySystemMode.HomeArmed
            ? modeHomeText
            : mode === SecuritySystemMode.NightArmed
              ? modeNightText
              : String(mode);

      const renderList = (list: string[]) =>
        list.length > 0 ? list.join(", ") : noneText;

      const text = (message || "")
        .replaceAll("${mode}", modeText)
        .replaceAll(
          "${seconds}",
          String(preactivationSeconds ?? autoDefuseSeconds ?? ""),
        )
      ...
      if (useRuleNotifiers) {
        ...
        for (const rule of this.activeRules ?? []) {
          for (const notifierId of rule.notifiers) {
            ...
            notifierPriority[notifierId] =
              rule.notifierData[notifierId]?.priority;
          }
        }
      ...
      for (const notifierId of notifiersToUse) {
        ...
        const notifier = sdk.systemManager.getDeviceById<
          Notifier & ScryptedDeviceBase
        >(notifierId);
        ...
        const isSupPriorityLow =
          supPriority &&
          [NotificationPriority.Low, NotificationPriority.SuperLow].includes(
            supPriority,
          );
        ...
        if (notifier.pluginId === PUSHOVER_PLUGIN_ID) {
          const priority = isSupPriorityLow
            ? supPriority === NotificationPriority.Low
              ? -1
              : -2
            : isCritical
              ? 1
          ...
          ntfyActions.push(
            ...alarmActions.slice(0, 3).map((action) => ({
              action: "http",
              label: action.title,
              url: action.url,
              method: "GET",
  ...
  async armSecuritySystem(mode: SecuritySystemMode): Promise<void> {
    ...
    const shouldStopArming =
      mode === SecuritySystemMode.Disarmed &&
      this.storageSettings.values.arming;
    ...
    if (mode === SecuritySystemMode.Disarmed) {
      ...
      try {
        ...
        const activeRules = this.plugin.allAvailableRules.filter((item) =>
          item.securitySystemModes?.includes(mode),
        );

        const activeRuleNames = activeRules.map((rule) => rule.name);
        logger.log(
          `${activeRules.length} rules found ${activeRuleNames.join(", ")}`,
        );
        ...
        this.storageSettings.values.activeNotifiers =
          Array.from(activeNotifiers);
        ...
        const activeDevices = Array.from(activeDevicesSet).map(
          (deviceId) => sdk.systemManager.getDeviceById(deviceId)?.name,
        );
        const bypassedDevices = Array.from(bypassedDevicesSet).map(
          (deviceId) => sdk.systemManager.getDeviceById(deviceId)?.name,
        );
        const blockingDevices = Array.from(blockingDevicesSet).map(
          (deviceId) => sdk.systemManager.getDeviceById(deviceId)?.name,
        );
        ...
        if (anyBlockers) {
          this.storageSettings.values.activeMode =
            this.securitySystemState.mode;
          this.securitySystemState = {
            ...this.securitySystemState,
          ...
          const activate = async () => {
            ...
            // Deactivation sequences of the previous mode (incl. Disarmed) only when the change is effective.
            if (previousMode && previousMode !== mode) {
              const { deactivation: previousDeactivationSequences } =
                getAlarmModeSequences({
                  mode: previousMode,
                  alarmStorage: this.storageSettings,
                  pluginStorage: this.plugin.storageSettings,
            ...
            // Run activation sequences BEFORE activating the mode
            ...
            const hasActivationSequences =
              activationSequences && activationSequences.length > 0;

            if (hasActivationSequences) {
              logger.log(
                `Running activation sequences before activating mode ${mode}`,
              );
              this.plugin
                .triggerRuleSequences({
              ...
              // Wait 5 seconds to allow sequences to start
          ...
          if (entity.preActivationTime) {
            ...
            this.activationListener = setTimeout(
