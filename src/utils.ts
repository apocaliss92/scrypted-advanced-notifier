...
export const EVENTS_RECORDER_PLUGIN_ID =
...
export const SCRYPTED_NVR_OBJECT_DETECTION_NAME =
...
export const cachedReaddir = async (path: string): Promise<string[]> => {
  ...
  if (
    readdirCache[path] &&
    now - readdirCache[path].timestamp < fsCacheDurationSeconds * 1000
  ) {
...
export const getAssetSource = (props: {
  videoUrl?: string;
  sourceId?: string;
}) => {
  ...
  const findFlags = () => {
    if (sourceId) {
      const eventsRecorderId = sdk.systemManager.getDeviceByName(
        EVENTS_RECORDER_PLUGIN_NAME,
      )?.id;
      const frigateBridgeId = sdk.systemManager.getDeviceByName(
        FRIGATE_BRIDGE_PLUGIN_NAME,
      )?.id;
      const advancedNotifierId = sdk.systemManager.getDeviceByName(
        ADVANCED_NOTIFIER_PLUGIN_NAME,
      )?.id;

      const isEventsRecorder =
        eventsRecorderId && eventsRecorderId === sourceId;
      const isFrigateBridge = frigateBridgeId && frigateBridgeId === sourceId;
      const isAdvancedNotifier =
        advancedNotifierId && advancedNotifierId === sourceId;
      const isNvr =
        !isEventsRecorder && !isFrigateBridge && !isAdvancedNotifier;
...
export const similarityConcidenceThresholdMap: Record<
  SimilarityConfidence,
  number
> = {
  [SimilarityConfidence.Low]: 0.22,
  [SimilarityConfidence.Medium]: 0.26,
  [SimilarityConfidence.High]: 0.3,
};
...
export const getElegibleDevices = (isFrigate?: boolean) => {
  const allDevices = Object.keys(sdk.systemManager.getSystemState()).map(
    (deviceId) => sdk.systemManager.getDeviceById<DeviceInterface>(deviceId),
  );
  ...
  return allDevices.filter((device) => {
    ...
    if (isFrigate) {
      return (
        mainSupported &&
        device.interfaces.includes(FRIGATE_OBJECT_DETECTOR_INTERFACE)
      );
    } else {
...
export const safeParseJson = <T = any>(
  maybeStringValue: string | object,
  fallback?: any,
) => {
  if (
    maybeStringValue === undefined ||
    maybeStringValue === null ||
    maybeStringValue === ""
  )
...
export const getAllAvailableUrls = async (hasCloudPlugin: boolean) => {
  ...
  if (hasCloudPlugin) {
    try {
      cloudSecureEndpoint = await sdk.endpointManager.getCloudEndpoint(
        undefined,
        { public: true },
      );
    } catch {}
  ...
  try {
    localSecureUrl = await sdk.endpointManager.getLocalEndpoint(undefined, {
      public: true,
    });
  } catch {}
  try {
    localInsecureUrl = await sdk.endpointManager.getLocalEndpoint(undefined, {
      public: true,
      insecure: true,
    });
  } catch {}
...
export const isDetectionRule = (rule: BaseRule) =>
  [RuleType.Audio, RuleType.Detection].includes(rule.ruleType);
...
export const getAssetsParams = async (props: {
  ...
  try {
    ...
    if (assetsOriginSource === AssetOriginSource.LocalSecure) {
      ...
      assetsOriginSource === AssetOriginSource.CloudSecure &&
      hasCloudPlugin
    ) {
      ...
      if (customOrigin) {
        ...
        logger.error(
          "Custom origin URL is not set, falling back to local secure endpoint.",
        );
        endpointRaw = localSecureUrl;
      }
    }
...
export const parseNvrNotificationMessage = async (
  cameraDevice: DeviceInterface,
  deviceSensors: string[],
  options?: NotifierOptions,
  console?: Console,
): Promise<ParseNotificationMessageResult> => {
  try {
    ...
    let allDetections: ObjectDetectionResult[] =
      options?.recordedEvent?.data?.detections ?? [];
    ...
    if (subtitle === "Offline") {
      ...
      const regex = new RegExp(
        "The (.*) has been offline for an extended period.",
      );
    ...
    if (
      [SupportedSensorType.Binary, SupportedSensorType.Lock].includes(
        eventType as SupportedSensorType,
      )
    ) {
      ...
      const foundSensor = deviceSensors.find((deviceId) => {
        ...
        if (device) {
          if (device.type === ScryptedDeviceType.Lock) {
            return (
              systemState[deviceId].lockState?.value === LockState.Unlocked
            );
          } else {
      ...
      if (foundSensor) {
        triggerDevice =
          sdk.systemManager.getDeviceById<DeviceInterface>(foundSensor);
      } else {
        console.log(
          `Trigger sensor not found: ${JSON.stringify({ deviceSensors })}`,
        );
      }
    }
    ...
    if (detection) {
      const allZones = uniq(
        allDetections
          .filter(
            (innerDetection) =>
              innerDetection.className === detection.className,
          )
    ...
    console.log(
      `Error parsing notification: ${JSON.stringify({ device: cameraDevice.name, options, deviceSensors })}`,
      e,
    );
...
export const getDetectionsPerZone = (detections: ObjectDetectionResult[]) => {
  ...
  for (const detection of detections ?? []) {
    ...
    if (
      detectionClass &&
      detectionClassForObjectsReporting.includes(detectionClass) &&
      score > 0.6
    ) {
...
export const filterAndSortValidDetections = (props: {
  ...
  const sortedByPriorityAndScore = sortBy(detections, (detection) => [
    detection?.className ? classnamePrio[detection.className] : 100,
    1 - (detection.score ?? 0),
  ]);
  ...
  const uniqueByClassName = uniqBy(
    sortedByPriorityAndScore,
    (det) => det.className,
  );
  ...
  const candidates = uniqueByClassName.filter((det) => {
    ...
    if (isLabel) {
      ...
      logger.debug(
        `Movement data ${JSON.stringify(movement)} not valid: ${JSON.stringify(det)}`,
      );
      return false;
    }
    ...
    if (
      ...
      [
        ...
        DetectionClass,
      ].includes(className as DetectionClass)
    ) {
      isSensorEvent = true;
    }
...
export const getTextSettings = (props: {
  forMixin: boolean;
  ...
  const settings: StorageSettingsDict<TextSettingKey> = {
    detectionTimeText: {
      ...
      type: "string",
      description:
        "Expression used to render the time shown in notifications. Available arguments ${time}",
    ...
    doorbellText: {
      ...
      type: "string",
      description:
        "Expression used to render the text when a vehicle is detected. Available arguments ${room} ${time}",
    ...
    doorWindowText: {
      ...
      type: "string",
      description:
        "Expression used to render the text when a binary sensor opens. Available arguments ${room} ${time} ${nvrLink}",
    ...
    doorlockText: {
      ...
      type: "string",
      description:
        "Expression used to render the text when a lock sensor opens. Available arguments ${room} ${time} ${nvrLink}",
    ...
    floodingText: {
      ...
      type: "string",
      description:
        "Expression used to render the text when a flood sensor opens. Available arguments ${room} ${time} ${nvrLink}",
    ...
    entrySensorText: {
      ...
      type: "string",
      description:
        "Expression used to render the text when an entry sensor opens. Available arguments ${room} ${time} ${nvrLink}",
    ...
    onlineText: {
      ...
      type: "string",
      description:
        "Expression used to render the text when a device comes back online. Available arguments ${time}",
    ...
    offlineText: {
      ...
      type: "string",
      description:
        "Expression used to render the text when a device goes offline. Available arguments ${time}",
    ...
    streamInterruptedText: {
      ...
      type: "string",
      description:
        "Expression used to render the text when a streams gets interrupted. Available arguments ${time} ${streamName}",
      defaultValue: !forMixin
        ? "Stream ${streamName} interrupted at ${time}"
        : undefined,
      placeholder: !forMixin
        ? "Stream ${streamName} interrupted at ${time}"
        : undefined,
    ...
    snoozeText: {
      ...
      type: "string",
      description:
        "Expression used to render the snooze texts. Available arguments ${timeText}",
...
export const ruleTypeMetadataMap: Record<
  RuleType,
  { rulesKey: string; rulePrefix: string; subgroupPrefix: string }
> = {
  [RuleType.Detection]: {
    rulePrefix: "rule",
    rulesKey: "detectionRules",
    subgroupPrefix: "DET",
  },
  [RuleType.Occupancy]: {
    rulePrefix: "occupancyRule",
    rulesKey: "occupancyRules",
    subgroupPrefix: "OCC",
  },
  [RuleType.Timelapse]: {
    rulePrefix: "timelapseRule",
    rulesKey: "timelapseRules",
    subgroupPrefix: "TIME",
  },
  [RuleType.Audio]: {
    rulePrefix: "audioRule",
    rulesKey: "audioRules",
    subgroupPrefix: "AUDIO",
  },
  [RuleType.Recording]: {
    rulePrefix: "recordingRule",
    rulesKey: "recordingRules",
    subgroupPrefix: "REC",
  },
  [RuleType.Patrol]: {
    rulePrefix: "patrolRule",
    rulesKey: "patrolRules",
    subgroupPrefix: "PAT",
  },
};
...
export const getMixinBaseSettings = (props: {
  ...
  try {
    ...
    const device = sdk.systemManager.getDeviceById<ScryptedDeviceBase>(
      mixin.id,
    );
    ...
    if (isCamera || isSensor) {
      settings.useNvrDetections = {
        ...
        description:
          "If enabled, the NVR notifications will be used (cropped images and more precise). If not raw detections will be used (snapshot will be roughlty taken at the beginning of the event) Make sure to extend the notifiers with this extension",
        type: "boolean",
        subgroup: "Detection",
        immediate: true,
        hide: true,
    ...
    if (isCamera) {
      const hasPtz = device?.interfaces?.includes(
    ...
    if (isCamera || isSensor) {
      settings.minDelayTime = {
        ...
        description:
          "Minimum amount of seconds to wait until a notification is sent. Set 0 to disable",
        type: "number",
...
export const getActiveRules = async (props: {
...
export const getEventTextKey = (props: {
  eventType: DetectionEvent;
  hasLabel: boolean;
}) => {
...
const ruleActionDeviceInterfacesMap: Partial<
  Record<RuleActionType, ScryptedInterface[]>
> = {
...
const notifierInterfaces: ScryptedInterface[] = [ScryptedInterface.Notifier];
...
type GetSpecificRules = (props: {
  group: string;
  subgroup: string;
  ruleName: string;
  showMore: boolean;
}) => Promise<StorageSetting[]>;
...
export const getNotifierData = (props: {
  ...
  const withActions =
    [HOMEASSISTANT_PLUGIN_ID, ZENTIK_PLUGIN_ID].includes(pluginId) &&
    isDetectionRule;
  ...
  const withOpenInApp = [HOMEASSISTANT_PLUGIN_ID, ZENTIK_PLUGIN_ID].includes(
    pluginId,
  );
  ...
  if (pluginId === HOMEASSISTANT_PLUGIN_ID) {
    ...
    priorityChoices.push(NotificationPriority.Low, NotificationPriority.Normal);
...
const getNotifierSettings = (props: {
  ...
  const prioritySetting: StorageSetting = {
    ...
    title: `Priority`,
    description:
      "Depends on the notifier, if High will always be a critical notification",
    group,
    subgroup,
  ...
  const actionsSetting: StorageSetting = {
    ...
    title: `Actions`,
    description:
      'I.e. {"action":"open_door","title":"Open door","icon":"sfsymbols:door", "url": "url"}',
    type: "string",
    multiple: true,
    group,
    subgroup,
  ...
  const openInAppSetting: StorageSetting = {
    key: openInAppKey,
    title:
      pluginId === HOMEASSISTANT_PLUGIN_ID
        ? `Open in the Homeassistant's Scrypted component`
  ...
  const addCameraActionsSetting: StorageSetting = {
    ...
    description:
      "Add to the notification the default actions defined on the camera",
    type: "boolean",
    group,
    subgroup,
    hide: !showMoreConfigurations,
...
export const getRuleSettings = async (props: {
  ...
  const cameraMixin = isCamera
    ? plugin.currentCameraMixinsMap[device.id]
    : undefined;

  const hasNvr = isPlugin
    ? !!plugin.nvrObjectDetectionDevice
    : cameraMixin
      ? cameraMixin.hasNvr
      : false;
  ...
  for (const ruleName of rules) {
    ...
    const currentActivation = safeParseJson<DetectionRuleActivation>(
      storage.getItem(activationKey),
    ...
    const showMoreConfigurations = safeParseJson<boolean>(
      storage.getItem(showMoreConfigurationsKey),
      false,
    );
    const aiEnabled = safeParseJson<boolean>(
      storage.getItem(aiEnabledKey),
      false,
    );
    const generateClip = safeParseJson<boolean>(
      storage.getItem(generateClipKey),
      false,
    );
    const notifiers = safeParseJson<string[]>(
      storage.getItem(notifiersKey),
      [],
    );
    const advancedSecurityEnabled = ruleType === RuleType.Detection;
    const isAdvancedSecuritySystem =
      advancedSecurityEnabled &&
      currentActivation === DetectionRuleActivation.AdvancedSecuritySystem;
      ...
      if (isCamera || isPlugin) {
        ...
        if (!hasNvr) {
          choices = choices.filter(
            (source) => source !== ScryptedEventSource.NVR,
          );
        }
        ...
        settings.push({
          ...
          description:
            "Select which detections should be used. The snapshots will come from the same source",
          type: "string",
          defaultValue: hasNvr
            ? ScryptedEventSource.NVR
      ...
      if (generateClip) {
        settings.push(
          ...
          },
          {
            ...
            title: "Clip type",
            description:
              "MP4 supported only by HA/Zentik notifiers, GIF supported mostly by everything",
            group,
            subgroup,
            immediate: true,
            type: "string",
          ...
          },
          {
            ...
            description:
              "Maximum time to extend existing clips instead of generating new ones",
            group,
            subgroup,
            type: "number",
            defaultValue: 30,
        ...
        settings.push(
          {
            ...
            description:
              "Set the post processing of the image. Default depends on the detection source, Crop for NVR and FullFrame for RawDetections",
            type: "string",
          ...
          },
          {
            ...
            description:
              "Highlight active zones in notifications or processing",
            group,
            subgroup,
            type: "boolean",
            immediate: true,
    ...
    if (
      currentActivation === DetectionRuleActivation.Schedule &&
    ...
    const specificSettings = await getSpecificRules({
      ruleName,
      subgroup,
      group,
      showMore: showMoreConfigurations,
    });
    ...
    if (!isOccupancyRule && !isRecordingRule) {
      settings.push({
        ...
        subgroup,
        type: "string",
        defaultValue: isAudioRule
          ? "Audio detected: ${decibels} dB for ${duration} seconds"
          : undefined,
    ...
    if (!isRecordingRule) {
      settings.push({
        ...
        description:
          "If enabled, snooze actions will act for any notifier of the rule, rather then per used device",
        type: "boolean",
        group,
        subgroup,
        immediate: true,
...
export const getSequencesSettings = async (props: {
  ...
  const { storage, refreshSettings, onTestSequence } = props;
  ...
  for (const sequenceName of sequenceNames) {
    ...
    const { actionsKey, minimumExecutionDelayKey, enabledKey, testKey } =
      getSequenceKeys({ sequenceName });
    const actionNames = safeParseJson<string[]>(
      storage.getItem(actionsKey),
      [],
    );
    ...
    for (const actionName of actionNames) {
      ...
      const currentType = safeParseJson<RuleActionType>(
        storage.getItem(typeKey),
      );
      ...
      if (currentType) {
        ...
        if (currentType === RuleActionType.Script) {
          const ruleActionDevicesFilter = getDeviceFilter(
            ruleActionDeviceInterfacesMap[currentType] || [],
          );
          ...
          if (device) {
            const foundDevice = sdk.systemManager.getDeviceById<PanTiltZoom>(
              device.id,
            );
            if (foundDevice) {
              if (currentType === RuleActionType.Ptz) {
                const presetNames = Object.entries(
                  foundDevice.ptzCapabilities?.presets || {},
...
export const getDetectionRulesSettings = async (props: {
  ...
  const { storage, device, ruleSource, refreshSettings, logger, plugin } =
    props;
  const isPlugin = ruleSource === RuleSource.Plugin;
  const { isCamera } = !isPlugin ? isDeviceSupported(device) : {};
  const cameraMixin = isCamera
  ...
  const getSpecificRules: GetSpecificRules = async ({
    group,
    ruleName,
    subgroup,
    showMore,
    ...
    const { detection, common } = getRuleKeys({
      ruleName,
    ...
    const clipDescription = safeParseJson<string>(
      storage.getItem(clipDescriptionKey),
      "",
    );
    const detectionClasses = safeParseJson<DetectionClass[]>(
      storage.getItem(detectionClassesKey),
      [],
    );
    const activationType = safeParseJson<DetectionRuleActivation>(
      storage.getItem(activationKey),
      DetectionRuleActivation.Always,
    );
    ...
    if (isPlugin) {
      ...
      if (isFrigate) {
        const { frigateAudioLabels, frigateObjectLabels, frigateZones } =
          await cameraMixin.getFrigateData();
    ...
    if (showCameraSettings) {
      ...
      if (
        (isRawDetection || isFrigate) &&
        detectionClasses.includes(DetectionClass.Audio)
      ) {
        settings.push({
          key: audioLabelsKey,
          title: "Audio labels",
      ...
      settings.push({
        ...
        title: "AI filter",
        description:
          "The prompt should be a question. This plugin will force the answer to be yes/no",
      ...
      settings.push({
        ...
        description:
          "Applied after detections. Threshold defined on the object detector will still take precedence, default to 0.5 for audio events, 0.7 otherwise",
        group,
        subgroup,
        type: "number",
        hide: !showMore,
      ...
      if (hasPlate) {
        settings.push(
          ...
          },
          {
            ...
            description:
              "Define how many characters the plate can differ (Whitespaces will not considered)",
            group,
            subgroup,
            type: "number",
            defaultValue: 2,
    ...
    const zonesDescription = isFrigate
      ? "Zones defined on the Frigate interface"
    ...
    let minDelayDescription =
      "Minimum amount of seconds to wait until a notification is sent for the same detection type.";
    if (isPlugin) {
    ...
    settings.push(
      ...
      },
      {
        ...
        description:
          "Set a value here in seconds to enable the camera recording when the rule is triggered. After the seconds specified, recording will be disabled",
        group,
        subgroup,
        type: "number",
        placeholder: "-",
...
export const nvrAcceleratedMotionSensorId = sdk.systemManager.getDeviceById(
  NVR_PLUGIN_ID,
  "motion",
)?.id;
...
export const getBaseRuleDefaults = (props: {
  ...
  const preClips = isOccupancy ? (isFrigate ? 20 : 12) : isFrigate ? 15 : 5;
...
export const getOccupancyRulesSettings = async (props: {
  ...
  const getSpecificRules: GetSpecificRules = async ({
    ...
    const { occupancy, common } = getRuleKeys({
      ruleName,
    ...
    settings.push(
      ...
      },
      {
        ...
        description:
          "In the final stages of check, confirm the result with AI to avoid false positives",
        group,
        subgroup,
        immediate: true,
        type: "boolean",
      ...
      },
      {
        ...
        description:
          "Seconds to wait until an occupancy state change gets confirmed",
        group,
        subgroup,
        type: "number",
      ...
      },
      {
        ...
        title: "Max objects",
        description:
          "Amount of objects that can fit the zone (if set to 2 and only 1 is detected, zone will be considered free)",
        group,
        subgroup,
        type: "number",
        placeholder: "1",
      ...
      },
      {
        ...
        description:
          "Text to use for the notification when the rule gets activated (zone occupied). Available arguments ${detectedObjects} ${maxObjects}",
        group,
        subgroup,
        type: "string",
      },
      {
        ...
        description:
          "Text to use for the notification when the rule gets deactivated (zone not occupied). Available arguments ${detectedObjects} ${maxObjects}",
        group,
        subgroup,
        type: "string",
      },
...
export const getTimelapseRulesSettings = async (props: {
  ...
  const getSpecificRules: GetSpecificRules = async ({
    ...
    const { timelapse, common } = getRuleKeys({
      ruleName,
      ruleType: RuleType.Timelapse,
    });
    ...
    settings.push(
      ...
      },
      {
        ...
        description:
          "Minimum amount of seconds to wait until a new frame is recorded",
        group,
        subgroup,
        type: "number",
        placeholder: "2",
...
export const getAudioRulesSettings = async (props: {
  ...
  const { plugin, storage, ruleSource, refreshSettings, logger, device } =
    props;
  ...
  const getSpecificRules: GetSpecificRules = async ({
    ...
    const { audio, common } = getRuleKeys({
      ruleName,
    ...
    settings.push(
      ...
      },
      {
        ...
        description:
          "How many seconds to analyze. (Basic object detector emits by default every 2 seconds)",
        group,
        subgroup,
        type: "number",
        placeholder: "6",
      ...
      },
      {
        ...
        description:
          "How many samples must hit de threshold to consider the notification",
        group,
        subgroup,
        type: "number",
        placeholder: "80",
...
export function getRecordingRules(props: {
  ...
  const deviceRules = safeParseJson<string[]>(
    deviceStorage?.getItem(rulesKey),
    [],
  );
  const pluginRules = safeParseJson<string[]>(
    pluginStorage?.getItem(rulesKey),
    [],
  );
  ...
  const processRules = (rules: string[], source: RuleSource) => {
    ...
    for (const ruleName of rules) {
      ...
      const detectionClasses = safeParseJson<string[]>(
        storage.getItem(recordingDetectionClassesKey),
        [],
      );
      const scoreThreshold = safeParseJson<number>(
        storage.getItem(recordingScoreThresholdKey),
      );
      const postEventSeconds = safeParseJson<number>(
        storage.getItem(postEventSecondsKey),
      );
      const maxClipLength = safeParseJson<number>(
        storage.getItem(maxClipLengthKey),
      );
      const prolongClipOnMotion = safeParseJson<boolean>(
        storage.getItem(prolongClipOnMotionKey),
        false,
      );
      const mainDevices = safeParseJson<string[]>(
        storage.getItem(devicesKey),
        [],
      );
      const allDevices = getElegibleDevices().map((device) => device.id);

      const devices = !isPlugin
        ? [deviceId]
        : mainDevices.length
          ? mainDevices
          : allDevices;
      ...
      const deviceOk =
        !!devices?.length && (deviceId ? devices.includes(deviceId) : true);
...
export const getRecordingRulesSettings = async (props: {
  ...
  const getSpecificRules: GetSpecificRules = async ({
    ...
    const { recording, common } = getRuleKeys({
      ruleName,
      ruleType: RuleType.Recording,
    });
...
export const isSchedulerActive = (props: {
  startTime: number;
  endTime: number;
}) => {
...
const initBasicRule = (props: {
  ...
  const { storage, ruleType, ruleName, ruleSource, securitySystem, logger } =
    props;
  ...
  const currentlyActive = safeParseJson<boolean>(
    storage.getItem(currentlyActiveKey),
    false,
  );
  ...
  const generateClipSpeed = safeParseJson<VideoclipSpeed>(
    storage.getItem(generateClipSpeedKey),
    VideoclipSpeed.Fast,
  );
  const securitySystemModes = safeParseJson<SecuritySystemMode[]>(
    storage.getItem(securitySystemModesKey),
    [],
  );
  ...
  const totalSnooze = safeParseJson<boolean>(
    storage.getItem(totalSnoozeKey),
    false,
  );
  const generateClipPostSeconds = safeParseJson<number>(
    storage.getItem(generateClipPostSecondsKey),
    postClips,
  );
  const generateClipPreSeconds = safeParseJson<number>(
    storage.getItem(generateClipPreSecondsKey),
    preClips,
  );
  const generateClipType = safeParseJson<VideoclipType>(
    storage.getItem(generateClipTypeKey),
    defaultVideoclipType,
  );
  const imageProcessing = safeParseJson<ImagePostProcessing>(
    storage.getItem(imageProcessingKey),
    ImagePostProcessing.Default,
  );
  const showActiveZones = safeParseJson<boolean>(
    storage.getItem(showActiveZonesKey),
    false,
  );
  const onActivationSequencesNames = safeParseJson<string[]>(
    storage.getItem(onActivationSequencesKey),
    [],
  );
  const onDeactivationSequencesNames = safeParseJson<string[]>(
    storage.getItem(onDeactivationSequencesKey),
    [],
  );
  const onTriggerSequencesNames = safeParseJson<string[]>(
    storage.getItem(onTriggerSequencesKey),
    [],
  );
  const onResetSequencesNames = safeParseJson<string[]>(
  ...
  for (const notifierId of notifiers) {
    ...
    const priority = safeParseJson<NotificationPriority>(
      storage.getItem(priorityKey),
      NotificationPriority.Normal,
    );
    const addSnooze = withSnoozing
      ? safeParseJson<boolean>(storage.getItem(addSnoozeKey), snoozingDefault)
      : false;
    const openInApp = withOpenInApp
      ? safeParseJson<boolean>(storage.getItem(openInAppKey), openInAppDefault)
      : false;
    const sound = withSound
      ? safeParseJson<string>(storage.getItem(soundKey))
      : undefined;
    const channel = withChannel
      ? safeParseJson<string>(storage.getItem(channelKey))
      : undefined;
    const notificationIcon = withNotificationIcon
      ? safeParseJson<string>(storage.getItem(notificationIconKey))
      : undefined;
    const iconColor = withIconColor
      ? safeParseJson<string>(storage.getItem(iconColorKey))
      : undefined;
    const addCameraActions = safeParseJson<boolean>(
      storage.getItem(addCameraActionsKey),
      addCameraActionsDefault,
    );
    const addDeleteNotificationAction = safeParseJson<boolean>(
      storage.getItem(deleteNotificationKey),
      withDeleteNotificationDefault,
    );
    const addClearNotificationAction = safeParseJson<boolean>(
      storage.getItem(clearNotificationKey),
      withClearNotificationDefault,
    );
    const addOpenNotificationAction = safeParseJson<boolean>(
      storage.getItem(openNotificationKey),
      withOpenNotificationDefault,
    );
    rule.notifierData[notifierId] = {
      actions: withActions
        ? actions.map((action) => safeParseJson(action))
        : [],
      priority,
      addSnooze,
      addCameraActions,
  ...
  if (
    activationType === DetectionRuleActivation.Schedule ||
    ruleType === RuleType.Timelapse
  ) {
  ...
  let sensorsOk = true;
  const enabledSensors = safeParseJson<string[]>(
    storage.getItem(enabledSensorsKey),
    [],
  );
  const disabledSensors = safeParseJson<string[]>(
    storage.getItem(disabledSensorsKey),
    [],
  );
  ...
  if (!!enabledSensors.length || !!disabledSensors.length) {
    if (!!enabledSensors.length) {
      sensorsOk = enabledSensors.every((sensorId) => {
        const sensorDevice =
          sdk.systemManager.getDeviceById<DeviceInterface>(sensorId);
        if (!sensorDevice) {
          return false;
        }
        ...
        if (!isSupported) {
          logger.error(
            `Sensor ${sensorDevice.name} (${sensorDevice.id}) of type ${sensorDevice.type} not supported`,
          );
          return false;
        }
  ...
  const isAdvancedSecuritySystem =
    activationType === DetectionRuleActivation.AdvancedSecuritySystem;
  ...
  if (securitySystemDevice && securitySystemModes?.length) {
    ...
    isSecuritySystemEnabled = currentMode
      ? securitySystemModes.includes(currentMode)
      : false;
  }

  const basicRuleAllowed =
    isEnabled && timeAllowed && sensorsOk && isSecuritySystemEnabled;
...
export const getSequenceObject = (props: {
  ...
  const { actionsKey, minimumExecutionDelayKey, enabledKey } = getSequenceKeys({
    sequenceName,
  });
  const actionNames = safeParseJson<string[]>(storage.getItem(actionsKey), []);
  const minimumExecutionDelay = safeParseJson<number>(
    storage.getItem(minimumExecutionDelayKey),
    15,
  );
...
export const getDetectionRules = (props: {
  ...
  const processDetectionRules = (
    storage: StorageSettings<any>,
    ruleSource: RuleSource,
  ) => {
    const detectionRuleNames = safeParseJson<string[]>(
      storage.getItem(rulesKey),
      [],
    );
    const isPlugin = ruleSource === RuleSource.Plugin;
    ...
    for (const detectionRuleName of detectionRuleNames) {
      ...
      const allDevices = getElegibleDevices(
        detectionSource === ScryptedEventSource.Frigate,
      ...
      const devices = !isPlugin
        ...
        : mainDevices.length
          ...
          : allDevices;
      const devicesToUse =
        activationType === DetectionRuleActivation.OnActive
          ? onActiveDevices
          : devices;

      const detectionClasses = safeParseJson<RuleDetectionClass[]>(
        storage.getItem(detectionClassesKey),
        [],
      );
      const isAudioOnly =
        detectionClasses.length === 1 &&
        detectionClasses[0] === DetectionClass.Audio;

      const nvrEvents = safeParseJson<NvrEvent[]>(
        storage.getItem(nvrEventsKey),
        [],
      );
      const audioLabels = safeParseJson<string[]>(
        storage.getItem(audioLabelsKey),
        [],
      );
      const animalLabels = safeParseJson<string[]>(
        storage.getItem(animalLabelsKey),
        [],
      );
      const vehicleLabels = safeParseJson<string[]>(
        storage.getItem(vehicleLabelsKey),
        [],
      );
      const scoreThreshold = safeParseJson<number>(
        storage.getItem(scoreThresholdKey),
        isAudioOnly ? 0.5 : 0.7,
      );
      ...
      const clipConfidence = safeParseJson<SimilarityConfidence>(
        storage.getItem(clipConfidenceKey),
        SimilarityConfidence.Medium,
      );
      const minMqttPublishDelay = safeParseJson<number>(
        storage.getItem(minMqttPublishDelayKey),
        15,
      );
      const disableNvrRecordingSeconds = safeParseJson<number>(
        storage.getItem(recordingTriggerSecondsKey),
      );
      const maxClipExtensionRange = safeParseJson<number>(
        storage.getItem(generateClipMaxExtensionRangeKey),
        30,
      );
      ...
      detectionRule.whitelistedZones = safeParseJson<string[]>(
        storage.getItem(whitelistedZonesKey),
        [],
      );
      detectionRule.blacklistedZones = safeParseJson<string[]>(
        storage.getItem(blacklistedZonesKey),
        [],
      );
      ...
      if (hasFace || hasPlate) {
        detectionRule.labelScoreThreshold = safeParseJson<number>(
          storage.getItem(labelScoreKey),
          0,
        );
      }

      if (hasFace) {
        detectionRule.people = safeParseJson<string[]>(
          storage.getItem(peopleKey),
          [],
        );
      }

      if (hasPlate) {
        detectionRule.plates = safeParseJson<string[]>(
          storage.getItem(platesKey),
          [],
        );
        detectionRule.plateMaxDistance = safeParseJson<number>(
          storage.getItem(plateMaxDistanceKey),
          0,
        );
      }
      ...
      if (isPlugin && device) {
        ...
        if (isSensor && !mainDevices.length) {
      ...
      const ruleAllowed = basicRuleAllowed && deviceOk && isSensorEnabled;

      console.debug(
        `Rule processed: ${JSON.stringify({
          detectionRule,
          ruleAllowed,
      ...
      if (
        deviceOk ||
        (isPlugin && activationType === DetectionRuleActivation.OnActive)
      ) {
        availableRules.push(cloneDeep(detectionRule));
      }

      const isFrigate = detectionSource === ScryptedEventSource.Frigate;
      const isRawDetection =
      ...
      if (ruleAllowed) {
        ...
        !anyAllowedNvrRule &&
          (anyAllowedNvrRule =
            detectionRule.detectionSource === ScryptedEventSource.NVR);
        !shouldListenDoorbell &&
          (shouldListenDoorbell = detectionClasses.includes(
            DetectionClass.Doorbell,
          ));
        !shouldClassifyAudio &&
          hasAudio &&
...
export const getDeviceOccupancyRules = (props: {
  deviceStorage?: StorageSettings<any>;
  pluginStorage?: StorageSettings<any>;
  device: DeviceBase;
}) => {
  ...
  const occupancyRuleNames = safeParseJson<string[]>(
    deviceStorage.getItem(rulesKey),
    [],
  );
  ...
  for (const occupancyRuleName of occupancyRuleNames) {
    ...
    const { confirmation, forceCheck } = getOccupancyDefaults(
      rule.detectionSource,
    );

    const zoneOccupiedText = safeParseJson<string>(
      deviceStorage.getItem(zoneOccupiedTextKey),
    );
    const zoneNotOccupiedText = safeParseJson<string>(
      deviceStorage.getItem(zoneNotOccupiedTextKey),
    );
    const detectionClass = safeParseJson<DetectionClass>(
      deviceStorage.getItem(detectionClassKey),
    );
    const scoreThreshold = safeParseJson<number>(
      deviceStorage.getItem(scoreThresholdKey),
      0.5,
    );
    const changeStateConfirm = safeParseJson<number>(
      deviceStorage.getItem(changeStateConfirmKey),
      confirmation,
    );
    const forceUpdate = safeParseJson<number>(
      deviceStorage.getItem(forceUpdateKey),
      forceCheck,
    );
    const maxObjects = safeParseJson<number>(
      deviceStorage.getItem(maxObjectsKey),
      1,
    );
    const observeZone = safeParseJson<string>(deviceStorage.getItem(zoneKey));
    const zoneMatchType = safeParseJson<ZoneMatchType>(
      deviceStorage.getItem(zoneMatchTypeKey),
      ZoneMatchType.Intersect,
    );
    const captureZone = safeParseJson<Point[]>(
      deviceStorage.getItem(captureZoneKey),
      [],
    );
    const occupies = safeParseJson<boolean>(
      deviceStorage.getItem(occupiesKey),
      false,
    );
    const confirmWithAi = safeParseJson<boolean>(
      deviceStorage.getItem(confirmWithAiKey),
      false,
    );
    const detectedObjects = safeParseJson<number>(
      deviceStorage.getItem(detectedObjectsKey),
    );
...
export const getDevicePatrolRules = (props: {
  ...
  const patrolRuleNames = safeParseJson<string[]>(
    deviceStorage.getItem(rulesKey),
    [],
  );
  ...
  for (const patrolRuleName of patrolRuleNames) {
    ...
    const presets = safeParseJson<string[]>(
      deviceStorage.getItem(patrolPresetsKey),
      [],
    );
    const minPresetSeconds = safeParseJson<number>(
      deviceStorage.getItem(patrolMinPresetSecondsKey),
      10,
    );
    const maxPresetSeconds = safeParseJson<number>(
      deviceStorage.getItem(patrolMaxPresetSecondsKey),
      30,
    );
    const blockingDetectionClasses = safeParseJson<DetectionClass[]>(
      deviceStorage.getItem(patrolBlockingDetectionClassesKey),
      [],
    );
    ...
    const supportsPtz = device.interfaces.includes(
      ScryptedInterface.PanTiltZoom,
    );
    ...
    const timingsOk =
      ...
      minPresetSeconds > 0 &&
      !!maxPresetSeconds &&
      maxPresetSeconds >= minPresetSeconds;

    const ruleAllowed =
      basicRuleAllowed && supportsPtz && hasPresets && timingsOk;
...
export const getPatrolRulesSettings = async (props: {
  ...
  const { storage, ruleSource, refreshSettings, logger, device, plugin } =
    props;
  ...
  const getSpecificRules: GetSpecificRules = async ({
    ...
    const presetNames = Object.values(
      device?.ptzCapabilities?.presets ?? {},
    ) as string[];
    ...
    settings.push(
      ...
      },
      {
        ...
        description:
          "If a blocking detection class matches, stay up to this time (from preset arrival)",
        group,
        subgroup,
        type: "number",
        immediate: true,
      ...
      },
      {
        ...
        description:
          "If any of these classes are detected while on a preset, the preset will be held up to the maximum dwell time",
        group,
        subgroup,
        type: "string",
        multiple: true,
...
export const getDeviceTimelapseRules = (props: {
  deviceStorage?: StorageSettings<any>;
  pluginStorage?: StorageSettings<any>;
  console: Console;
  device: DeviceBase;
  ...
  const timelapseRuleNames = safeParseJson<string[]>(
    deviceStorage.getItem(rulesKey),
    [],
  );
  for (const timelapseRuleName of timelapseRuleNames) {
    ...
    const minDelay = safeParseJson<number>(
      deviceStorage.getItem(framesAcquisitionDelayKey),
    );
    const timelapseFramerate = safeParseJson<number>(
      deviceStorage.getItem(timelapseFramerateKey),
    );
    const lastGenerated = safeParseJson<number>(
      deviceStorage.getItem(lastGeneratedKey),
    );
    const regularSnapshotInterval = safeParseJson<number>(
      deviceStorage.getItem(regularSnapshotIntervalKey),
    );
    ...
    console.debug(
      `Timelapse rule processed: ${JSON.stringify({
        timelapseRule,
        basicRuleAllowed,
      })}`,
    );
...
export const getDeviceAudioRules = (props: {
  deviceStorage?: StorageSettings<any>;
  pluginStorage?: StorageSettings<any>;
  console: Console;
  device: DeviceBase;
  ...
  if (device.interfaces.includes(ScryptedInterface.AudioVolumeControl)) {
    ...
    const audioRuleNames = safeParseJson<string[]>(
      deviceStorage.getItem(rulesKey),
      [],
    );
    for (const audioRuleName of audioRuleNames) {
      const {
        common: { textKey, minDelayKey },
        audio: { decibelThresholdKey, audioDurationKey, hitPercentageKey },
      } = getRuleKeys({
        ruleType: RuleType.Audio,
      ...
      const customText = safeParseJson<string>(deviceStorage.getItem(textKey));
      const decibelThreshold = safeParseJson<number>(
        deviceStorage.getItem(decibelThresholdKey),
        -30,
      );
      const audioDuration = safeParseJson<number>(
        deviceStorage.getItem(audioDurationKey),
        6,
      );
      const hitPercentage = safeParseJson<number>(
        deviceStorage.getItem(hitPercentageKey),
        80,
      );
      const minDelay = safeParseJson<number>(
        deviceStorage.getItem(minDelayKey),
      );
      ...
      console.debug(
        `Audio rule processed: ${JSON.stringify({
          audioRule,
          basicRuleAllowed,
        })}`,
      );
...
export const getAllDevices = () => {
  return Object.keys(sdk.systemManager.getSystemState()).map((id) =>
    sdk.systemManager.getDeviceById(id),
  );
};
...
export const convertSettingsToStorageSettings = async (props: {
  ...
  const settings: StorageSetting[] = await new StorageSettings(
    device,
    initStorage,
  ).getSettings();
...
export const isDeviceSupported = (device: DeviceBase) => {
  ...
  const isBinarySensor =
    !isCamera && interfaces.includes(ScryptedInterface.BinarySensor);
  ...
  const sensorType: SupportedSensorType = isLock
...
export const binarySensorMetadataMap: Record<
  SupportedSensorType,
...
> = {
  ...
  [SupportedSensorType.Lock]: {
    ...
    isActiveFn: (device, value) =>
      (device?.lockState ?? value) === LockState.Unlocked,
  },
  [SupportedSensorType.Entry]: {
...
export const splitRules = (props: {
  ...
  for (const rule of allRules) {
    const isCurrentlyActive = currentlyRunningRules.find(
      (innerRule) => rule.name === innerRule.name,
    );
    const shouldBeActive = rulesToActivate.find(
      (innerRule) => rule.name === innerRule.name,
    );
    ...
    if (shouldBeActive && (!isCurrentlyActive || !rule.currentlyActive)) {
      ...
      !shouldBeActive &&
      (isCurrentlyActive || (rule.currentlyActive && !isPluginForDevice))
    ) {
      rulesToDisable.push(cloneDeep(rule));
    }
  }

  return [rulesToEnable, rulesToDisable];
};
...
export const getSnoozeId = (props: {
  ...
  if (rule?.totalSnooze) {
    return `${cameraId}_${specificIdentifier}`;
  } else {
...
export const getB64ImageLog = (b64Image: string) =>
  `${b64Image ? b64Image?.substring(0, 10) + "..." : "NO_IMAGE"}`;
export const getUrlLog = (url?: string) =>
  `${url ? url?.substring(0, 10) + "..." : "NO_URL"}`;
export const getDetectionsLog = (detections: ObjectDetectionResult[]) =>
  uniq(
    detections.map(
      (item) => `${item.className}${item.label ? "-" + item.label : ""}`,
    ),
  ).join(", ");
export const getDetectionsLogShort = (detections: ObjectDetectionResult[]) =>
  uniq(detections.map((item) => `${item.label ?? item.className}`)).join("_");
export const getRulesLog = (rulesToUpdate: MatchRule[]) =>
  uniq(rulesToUpdate.map(getDetectionKey)).join(", ");
...
export const haSnoozeAutomation = {
  ...
  description:
    "Automation auto-generated by the scrypted's plugin Advanced notifier to handle snooze actions",
  trigger: [
    {
      platform: "event",
  ...
  condition: [
    {
      condition: "template",
      value_template:
        "{{ (trigger.event.data.action_name is defined and trigger.event.data.action_name is match('scrypted_an_snooze_.*')) or (trigger.event.data.action is defined and trigger.event.data.action is match('scrypted_an_snooze.*')) }}",
    },
  ],
  action: [
    {
      variables: {
        event_name:
          "{% if 'action' in trigger.event.data %}{{ trigger.event.data.action }}{% else %}{{ trigger.event.data.actionName }}{% endif %}",
        suffix:
          "{% set prefix = 'scrypted_an_snooze_' %} {{ event_name[prefix|length:] }}",
...
export const moToB64 = async (mo: MediaObject) => {
  const bufferImage = await sdk.mediaManager.convertMediaObjectToBuffer(
    mo,
    "image/jpeg",
  );
...
export const getEmbeddingSimilarityScore = async (props: {
  ...
  const clipDevice = sdk.systemManager.getDeviceById<
...
export const calculateSize = async (props: {
  ...
  const calculateSizeInner = async (innerPath: string) => {
    const entries = await fs.promises.readdir(innerPath, {
      withFileTypes: true,
    });
    const filteredFiles = filenamePrefix
      ? entries.filter((entry) => entry.name.includes(filenamePrefix))
      : entries;
