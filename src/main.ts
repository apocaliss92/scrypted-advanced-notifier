...
export default class AdvancedNotifierPlugin extends BasePlugin implements MixinProvider, HttpRequestHandler, DeviceProvider {
    ...
    async onRequest(request: HttpRequest, response: HttpResponse): Promise<void> {
        ...
        try {
            const { lastSnapshot, haAction, timelapseDownload, timelapseStream, timelapseThumbnail } = await getWebooks();
            if (webhook === haAction) {
    ...
    getLogger(device?: DeviceInterface) {
        ...
        if (device) {
            logger = this.currentMixinsMap[device.name]?.getLogger() ?? logger;
        }
